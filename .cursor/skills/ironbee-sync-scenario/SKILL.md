...
Re-validate + repair saved verification **scenarios** using the devtools `scenario-*` MCP tools
...
1. **Resolve mode + target**: strip a leading `check` token (→ dry-run) and a leading `force` token
   ...
   Run targets that share an `ironbee.group` in ascending `ironbee.order` (a multi-step flow kept as separate sequenced scenarios — earlier steps set up state later ones need).
2. **For each target scenario** (resolve via `scenario-search` / `scenario-list`; `all` = the stale
   ones — covered files changed since their `ironbee.commit`, or authored as drafts) **run it**
   (`scenario-run`, against the live app — start it if needed, tear down what you started) and classify:
   - **passes** → still current; (non-check) `scenario-update` to stamp `ironbee.commit` → HEAD
     (read via `git rev-parse HEAD`) + `ironbee.liveValidated: true`. `scenario-update`
   ...
   - **mechanical DRIFT** (the way to reach / drive the flow changed, not the expected outcome) →
     repair the SCRIPT mechanics only, `scenario-update`, re-run until green, then stamp.
